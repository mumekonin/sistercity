...
import { City, NewsApprovalStatus, NewsPostedByCity, Role } from "src/common/enum/enum";
...
export class NewsService {
  ...
  async updateNews(id: string, updateNewsDto: UpdateNewsDto, files: Express.Multer.File[], currentUser: any): Promise<NewsResponse> {
    ...
    if (currentUser.role === Role.CITY_ADMIN && news.postedByCity !== currentUser.city && news.postedByCity !== NewsPostedByCity.JOINT) {
      throw new ForbiddenException('You can only manage news posted by your city');
    }
  ...
  async getAllNews(category?: string, city?: string, page: number = 1, limit: number = 10) {
    ...
    const news = await this.newsModel.find(filter).sort({ publishedAt: -1 }).skip(skip).limit(limit).lean();
